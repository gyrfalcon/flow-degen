...
*** Breaking
    ...
    3. =MetaType= has been expanded to include use cases like
       =$PropertyType<T, k>= where =k= is a string literal and not a type. This
       includes the addition of a =literal= bool field to differentiate between
       literal =MetaType=s and non-literal =MetaType=s. Consumers will need to
       update local =MetaType=s to add this property.
*** Additions
